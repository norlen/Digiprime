...
nameForm.addEventListener('submit', (event) => {
	...
	const selectCostumer = document.querySelector('#costumer').value.toLowerCase();
	const selectReferenceSector = document.querySelector('#referenceSector').value.toLowerCase();
	const selectReferenceType = document.querySelector('#referenceType').value.toLowerCase();
	...
	Array.from(referenceSectors).forEach(function(referenceSector) {
		...
		//console.log(searchResult)
		if (searchResult.toLowerCase().indexOf(selectReferenceSector) != -1 && referenceSector.closest('.remanents')) {
	...
	Array.from(referenceTypes).forEach(function(referenceType) {
		...
		//console.log(searchResult)
		if (searchResult.toLowerCase().indexOf(selectReferenceType) != -1 && referenceType.closest('.remanentsBlock')) {
