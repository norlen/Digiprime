...
if (!AUTH_API_KEY) {
  throw new Error("Environment variable AUTH_API_KEY must be set");
}
...
/**
 * Verifies a JWT token. This should be called before every operation using
 * a user's identify.
 *
 * ## Errors
 * - Assuming we get a 401 if the JWT is invalid? TODO: check after it has expired.
 *
 * ## Example
 *
 * Function can be called as
 ...
 * ```
 *
 * A successful verify may return
 * ```json
 * {
 *     "user": {
 *         "uuid": "uuid",
 ...
 * }>}
 */
module.exports.verify = async (jwt) => {
  const url = `${AUTH_BASE_URL}/user/verify`;
  const headers = {
    APIKEY: AUTH_API_KEY,
    Authorization: jwt,
    endUserAuthorization: jwt,
  };
