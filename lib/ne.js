...
module.exports.joinAuction = async (username, auctionId) => {
 ...
 *          total count.
 */
module.exports.listPublic = async (username, page = 0, perPage = 20) => {
  const skip = (page - 1) * perPage;
  const url = `${NE_BASE_URL}/rooms/public?skip=${skip}&limit=${perPage}`;
  const auth = { username };

  const response = await axios.get(url, { auth });
  const { rooms, count } = response.data;
